...
#import "ChartLeftZhuRightZheView.h"
#import "LineBarChart.h"

#import "PopChartView.h"

@interface ChartLeftZhuRightZheView()<UIGestureRecognizerDelegate>
...
@end
@implementation ChartLeftZhuRightZheView
...
-(void)passTheTime:(NSString *)time andLeftTitle:(NSString *)leftTitle  withLeftData:(NSArray *)leftData withLMax:(CGFloat)lmax withLMin:(CGFloat)lmin andRightTitle:(NSString *)rightTitle withRightData:(NSArray *)rightData withRMax:(CGFloat)rmax withRMin:(CGFloat)rmin andYCount:(NSInteger)yCount andLeftColor:(NSString *)leftColor andRighttColor:(NSString *)rightColor andDanwei:(NSString *)danwei{
    ...
    self.rightData=[NSMutableArray arrayWithArray:@[@0.086, @-0.027999999999999997, @-0.006999999999999999, @0.013000000000000001, @-0.023, @-0.018000000000000002, @0.06, @-0.042, @-0.05, @0, @-0.1, @0.1]];
            ...
            self.lineBarChart.lineBarDataSet.gridConfig.bottomLableAxis.lables =self.xData;
    self.lineBarChart.lineBarDataSet.gridConfig.leftNumberAxis.splitCount = self.yCount;
    self.lineBarChart.lineBarDataSet.gridConfig.rightNumberAxis.splitCount = self.yCount;
    self.lineBarChart.lineBarDataSet.gridConfig.leftNumberAxis.max=[NSNumber numberWithFloat: self.lMax];
    self.lineBarChart.lineBarDataSet.gridConfig.leftNumberAxis.min=[NSNumber numberWithFloat: self.lMin];
    self.lineBarChart.lineBarDataSet.gridConfig.rightNumberAxis.max=[NSNumber numberWithFloat: self.rMax];
    self.lineBarChart.lineBarDataSet.gridConfig.rightNumberAxis.min=[NSNumber numberWithFloat: self.rMin];
    ...
    self.rightSmallV.layer.borderColor= self.rightColor.length?[self colorWithHexStr:self.rightColor ].CGColor:[UIColor greenColor].CGColor;
    self.rightSmallV.layer.masksToBounds=YES;
     self.rightSmallL.text=self.rightTitle;
...
-(void)zhuzhuangzhexian{
    ...
    lineBarSet.gridConfig.leftNumberAxis.splitCount = 5;
    lineBarSet.gridConfig.leftNumberAxis.max=@1000;
    lineBarSet.gridConfig.leftNumberAxis.min=@0;
    ...
    _leftSmallL=[[UILabel alloc]initWithFrame:CGRectMake((self.bounds.size.width/2)-100, 310, 80, 15)];
    ...
    _rightSmallV=[[UIView alloc]initWithFrame:CGRectMake((self.bounds.size.width/2), 312.5, 10, 10)];
    _rightSmallV.backgroundColor=[UIColor whiteColor];
    _rightSmallV.layer.borderWidth=2;
    _rightSmallV.layer.cornerRadius=5;
    [self addSubview:_rightSmallV];
    
    _rightSmallL=[[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(_rightSmallV.frame)+5, 310, 80, 15)];
    _rightSmallL.textColor=[UIColor blackColor];
    _rightSmallL.font=[UIFont systemFontOfSize:13];
     [self addSubview:_rightSmallL];
